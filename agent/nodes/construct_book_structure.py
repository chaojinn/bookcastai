...
def make_construct_book_structure_node() -> Callable[["EPUBAgentState"], "EPUBAgentState"]:
    def construct_book_structure(state: "EPUBAgentState") -> "EPUBAgentState":
        ...
        selected_numbers = _query_openroute_for_chapters(preview_items)
        if selected_numbers:
            updated_chapters = [
                chapter
                for chapter in chapters
...
def _query_openroute_for_chapters(preview_items: Sequence[dict[str, object]]) -> List[int]:
    ...
    logger.info("OpenRoute chapter selection request: %s", request_payload)
    cached_content = _get_cached_openroute_response(request_payload)
...
def _openroute_cache_key(payload: Dict[str, Any]) -> str:
    ...
    logger.info("OpenRoute chapter selection output: %s", content)
    try:
        raw_numbers = json.loads(content)
    except json.JSONDecodeError:
        logger.error("OpenRoute selection did not return valid JSON. Content: %s", content)
